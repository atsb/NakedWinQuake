...
void Cache_Move(cache_system_t* c)
{
	...
	if (new_cs)
	{
		...
		Cache_Free(c->user); //johnfitz -- added second argument
	...
	else
	{
		...
		Cache_Free(c->user); // tough luck... //johnfitz -- added second argument
	}
}
...
void Cache_FreeHigh(int new_high_hunk)
{
	...
	while (1)
	{
		...
		if ((byte*)c + c->size <= hunk_base + hunk_size - new_high_hunk)
			return;		// there is space to grow the hunk
		if (c == prev)
			Cache_Free(c->user);	// didn't move out of the way //johnfitz -- added second argument
		else
		{
...
void Cache_Flush(void)
{
	while (cache_head.next != &cache_head)
		Cache_Free(cache_head.next->user); // reclaim the space //johnfitz -- added second argument
}
...
void* Cache_Alloc(cache_user_t* c, int size, const char* name)
{
	...
	while (1)
	{
		...
		Cache_Free(cache_head.lru_prev->user); //johnfitz -- added second argument
	}
